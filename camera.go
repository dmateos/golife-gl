...
func (c *Camera) Orientation() mgl32.Mat4 {
	orientation := mgl32.HomogRotate3D(c.vertAngle, mgl32.Vec3{1, 0, 0})
	orientation = orientation.Mul4(mgl32.HomogRotate3D(c.horizAngle, mgl32.Vec3{0, 1, 0}))
	return orientation
}
...
func (c *Camera) OffsetPosition(offset mgl32.Vec3) {
	c.pos = c.pos.Add(offset)
}

func (c *Camera) Forward(step float32) mgl32.Vec3 {
	vec := c.Orientation().Inv().Mul4x1(mgl32.Vec4{0, 0, -step, 1})
	return mgl32.Vec3{vec.X(), vec.Y(), vec.Z()}
}

func (c *Camera) Right(step float32) mgl32.Vec3 {
	vec := c.Orientation().Inv().Mul4x1(mgl32.Vec4{step, 0, 0, 1})
	return mgl32.Vec3{vec.X(), vec.Y(), vec.Z()}
}

func (c *Camera) Up(step float32) mgl32.Vec3 {
	vec := c.Orientation().Inv().Mul4x1(mgl32.Vec4{0, step, 0, 1})
	return mgl32.Vec3{vec.X(), vec.Y(), vec.Z()}
}
